    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != ResponseBean.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                response.close();
                //noinspection unchecked
                return (T) simpleResponse.toResponseBean();
            } else {
                // 泛型格式如下： new JsonCallback<LzyResponse<内层JavaBean>>(this)
                ResponseBean responseBean = Convert.fromJson(jsonReader, type);
                response.close();
                int code = responseBean.status;
                //0表示成功，其余的表示失败，这里根据实际情况修改
                if (code == 0) {
                    //noinspection unchecked
...
//                    throw new RepExecpetion("错误代码：" + code + "，错误信息：" + responseBean.msg);
                    throw new RepException(Convert.toJson(responseBean));
                }
            }
        }
    }
